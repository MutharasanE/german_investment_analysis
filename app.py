...
def main():
    ...
    sp500_tickers = [t for t in tickers if not t.endswith(".DE")]

    controls_left, controls_right = st.columns([1.2, 1])
    with controls_left:
        selected_index = st.selectbox("Select index", ["DAX", "S&P 500"])

        index_tickers = dax_tickers if selected_index == "DAX" else sp500_tickers
        if not index_tickers:
            st.warning(f"No {selected_index} tickers are available in the current dataset.")
            return

        ticker_options = {ticker_display(t): t for t in index_tickers}
    ...
    with st.expander("📖 Feature Glossary & Causal Graph (Click to expand)"):
        st.markdown("""
        **How features are calculated:**
        ...
        if causal_graph_path.exists():
            st.image(str(causal_graph_path), caption="DirectLiNGAM Mathematical Causal Graph", use_container_width=True)
    ...
    with st.form("minimal_survey_form", clear_on_submit=True):
        ...
        st.markdown("<div class='survey-question'>Which explanation better tells you what to change to alter the decision?</div>", unsafe_allow_html=True)
        actionability_display = st.radio(
            "Actionability",
            st.session_state[order_key],
            horizontal=True,
            label_visibility="collapsed",
        ...
        if submitted:
            ...
            actionability_label = "A" if actionability_display == "Explanation A" else "B" if actionability_display == "Explanation B" else "No"
            save_vote(
                ...
                survey_variant="minimal_unbiased_single_page_v2_counterfactual",
                actionability_choice=actionability_label,
                trust_score=trust_score,
                confidence_score=confidence_score,
